...
# Routes
# /, profile, settings, help, feed, download-report, input, predexcel
...
def save_csv_files():
    ...
    try:
        ...
        try:
            ...
            # Store data preview in session
            session['df1_html'] = df1.to_html(classes="table table-bordered", index=False)
            session['df2_html'] = df2.to_html(classes="table table-bordered", index=False)
            ...
            return render_template("index.html", df1_html=session['df1_html'], df2_html=session['df2_html'])
...
def predexcel():
    ...
    try:
        ...
        prediction_df = pd.read_csv(prediction_path)
        prediction_html = prediction_df.to_html(classes="table table-bordered", index=False)

        return render_template(
            "index1.html",
            prediction_html=prediction_html,
            df1_html=session.get('df1_html', ""),
            df2_html=session.get('df2_html', "")
        )
